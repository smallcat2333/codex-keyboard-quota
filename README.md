...
![V0.2.0 中转站余额模式：紧凑状态区、两行操作区与独立运行详情](docs/images/v0.2.0-dashboard.png)

官方额度模式：

![V0.2.0 官方额度模式：5小时额度、周额度及重置沙漏](docs/images/v0.2.0-official-quota.png)

额度状态置于右上方，连接状态横排显示；操作区只占两行，底部剩余空间完整留给可独立滚动的运行详情。
...
- 窗口顶部显示 24×8 彩色点阵，每秒只读同步本工具最近成功写屏的记录；与键盘共用帧渲染逻辑，后台查询或采样变化不会提前改变预览。键盘断电、重连或其它软件改屏无法通过该记录确认，需重新「测试显示」。
- 右侧额度状态下方显示同币种的十个 30 分钟消耗区间，分两排、从左到右及从上到下排列，右下角为当前周期；悬停起点可查看完整日期。统计读取每分钟采样的最新持久化记录，独立于写屏阈值；缺失或中断周期显示 `--`，停止采样超过两分钟时提示更新后台程序，官方模式不显示金额统计。未部署时可用「测试显示」采样；窗口监控本身不查询网络、不写键盘。
...
- `2026-09-11 | 0.2.0 | 紧凑双列布局：额度状态移至右上，连接状态横排，操作区压缩为两行，底部运行详情独立滚动；消耗统计展示采样时间并识别停止更新；同步 GUI/EXE 版本与新版实机截图。`

- `2026-09-11 | 0.1.0 | Rust 窗口新增每秒同步的 24×8 写屏点阵预览、十个半小时消耗区间及当前累计；只读快照沿用跨进程刷新锁，区分已写屏内容与最新采样。`
