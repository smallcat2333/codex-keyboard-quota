...
- 每根柱统计 30 分钟累计消耗（不是每分钟平均），最右侧为当前周期累计，左侧为最近九个已完成周期；随余额刷新显示最新状态，不等待当前周期结束。每满 1 USD 从底部向上点亮一个点：`<1` 为 0 点，`[1,2)` 为 1 点绿色，`[2,3)` 为 2 点绿色，`[3,4)` 为 3 点蓝色，`[4,5)` 为 4 点橙色，`≥5` 为 5 点整柱红色。0 档不点亮，最多 5 点。
- 每分钟采样，各供应商保留独立余额基线，同币种供应商共用消耗时间轴。例如半小时内 A 消耗 2 USD、切到 B 后消耗 1 USD，该柱合计 3 USD。切换不清空柱图或重置周期，切入（含切回）供应商时重新建立对应基线，不计算跨供应商差值，也不补计停用期间的余额变化。充值不抵扣已记录的消耗，不同币种不混加。
- 共用时间轴与余额基线写入注册表，跨进程/重启保留；升级后的共用时间轴从首次采样开始，不合并旧版各供应商的独立柱图。查询失败或采样间隔超过 2 分钟的周期留空，不伪造消耗。余额下降发生在两次采样之间，周期边界的一次采样归入刚结束的周期。
...
- 消耗柱变化或满 30 分钟均不单独触发写屏，只随余额达到刷新规则、供应商切换或手动测试显示一起更新。余额封顶显示 `999` 时仍使用真实余额计算消耗。
...
- `2026-09-11 | 0.1.0 | 消耗柱每满 1 USD 点亮一点、5 USD 及以上封顶五点；最右柱显示当前周期累计，只跟随余额刷新，不再独立触发写屏。`

- `2026-09-11 | 0.1.0 | 各供应商余额基线独立，同币种消耗合并到共用半小时时间轴，切换保留柱图和周期并避免跨供应商余额相减。`
